...
charSeries (const CFList& L)
{
  ...
  while (!tmp.isEmpty())
  {
    ...
    if (count > 0)
      ppi= Union (ppi1, ListCFList (l), operator==);
    else
      ppi= ListCFList();
    ...
    {
      result= Union (ListCFList (charset), result, operator==);
...
irrCharSeries (const CFList & PS)
{
  ...
  while (!qhi.isEmpty())
  {
    ...
    {
      nr_of_iteration += 1;
      ppi= Union (ppi1, ListCFList (qs), operator==);
    }
    ...
    if (!cs.isEmpty() && cs.getFirst().level() > 0)
    {
      ...
      if (indexRed <= 0) // irreducible
      {
        ...
        if (!find (pi, cs, operator==))
        {
          pi= Union (ListCFList (cs), pi, operator==);
          if (cs.getFirst().level() > 0)
          {
            ...
            if (indexRed <= 0) //irreducible
            {
              qsi= Union (ListCFList(cs), qsi, operator==);
      ...
      if (indexRed > 0)
      {
        ...
        if (indexRed > 1)
        {
          ...
          iss= Union (adjoinb (ts, qs, qqi, cst), adjoin (is, qs, qqi), operator==);
        }
        else
