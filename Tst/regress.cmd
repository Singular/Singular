...
$ignore = "-b -I '^\/\/.*used time:' -I '^\/\/.*ignore:' -I '? error occurred in'";
$keep = "no";
$verbosity = 1;
$generate = "no";
$exit_code = 0;
...
sub tst_check
{
  ...
  {
    & mysystem("$gzip -f $root.res; $uuencode $root.res.gz $root.res.gz > $root.res.gz.uu; $rm -rf $root.res.gz $root.new.res $root.diff");
    if ($keep eq "yes")
    {
