...
{
  print(lv);
  write("MPfile:w /tmp/t.mp", lv);
  return(lv == read("MPfile: /tmp/t.mp"));
};
...
module mv;
module mv1 = v, v1, v2;
module mv2 = v1+v2, p1*v2, p2*(v1-v2), v, v1, v2;
...
dump("MPfile:w /tmp/mptest");
LIB "general.lib";
killall();
killall("proc");
getdump("MPfile: /tmp/mptest");

//link ll = "MPtcp:";
//open(ll);
//write(ll, quote(getdump(mp_ll)));
//dump(ll);
//read(ll);
//LIB "general.lib";
//killall(typeof(ll));
//killall("proc");
//write(ll, quote(dump(mp_ll)));
//getdump(ll);
//read(ll);
//close(ll);
dump("");
