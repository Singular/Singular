 ...
 *  Created: 11/99
 *  Version: $Id: omCheck.c,v 1.1 1999-11-23 20:40:12 obachman Exp $
 *******************************************************************/
#include "omConfig.h"
#include "omPrivate.h"
#include "omLocal.h"
 ...
 *  
 *******************************************************************/
static int omCheckBinAddrSize(void* addr, size_t size, int level);
static int omCheckPrintBinError(const char* msg, void* addr, size_t size,
                            omBin bin);
static const char* omCheckTestBinAddrSize(void* addr, size_t size, int level, 
                                      omBin *bin);
static const char* omCheckTestBin(omBin bin, int level);
static const char* omCheckTestBinPage(omBinPage page, int level);
static int   omCheckIsKnownTopBin(omBin bin);
...
{
  return omCheckBinAddrSize(addr, 0, level);
}
...
int omCheckBlockAddr(void* addr, size_t size, int level)
{
  ...
  if (size <= OM_MAX_BLOCK_SIZE)
    return omCheckBinAddrSize(addr, size, level);
  else
    return 1;
}

int omCheckChunkAddr(void* addr, int level)
{
  omBinPage page;
  addr = addr - SIZEOF_OM_ALIGNMENT;
  page = *((omBinPage*) addr);
  if (page != om_LargePage)
    return omCheckBinAddrSize(addr, -1, level);
  else
    return 1;
}
...
{
  const char* msg = omTestBin(bin, level);
  if (msg != NULL) 
  {
    omPrintBinError(msg, NULL, 0, bin);
    return 0;
  }
  return 1;
}
...
int omCheckBins(int level)
{
  ...
  else
  {
    ...
    for (i=0; i<= OM_MAX_BIN_INDEX; i++)
    {
      if (! omCheckBin(&om_StaticBin[i], level)) return 0;
    }
    s_bin = om_SpecBin;
    while (s_bin != NULL)
    {
      if (! omCheckBin(s_bin->bin, level)) return 0;
      s_bin = s_bin->next;
    }
    return 1;
  }
...
void* omCheckAllocBin(omBin bin, const int zero, int level)
{
  void* res;
  
  if (om_Check > level) level = om_Check;
...
void  omCheckFreeBin(void* addr, int level)
{
  ...
  if (omCheckBinAddr(addr, level))
    __omFreeBin(addr);

  omCheckBins(level - 2);
}

void* omCheckAllocBlock(size_t size, const int zero, int level)
{
  void* res;
  ...
  if (size <= 0) 
  {
    omError("requested AllocBlock size <= 0");
    size = 1;
  }
  ...
  omCheckBlockAddr(res, size, level - 1);
  omCheckBins(level - 2);
...
void omCheckFreeBlock(void* addr, size_t size, int level)
{
  if (om_Check > level) level = om_Check;
  omCheckBins(level - 2);
  if (! omCheckBlockAddr(addr, size, level)) return;
...
void* omCheckAllocChunk(size_t size, const int zero, int level)
{
  void* res;
...
{
  if (om_Check > level) level = om_Check;
  if (! omCheckChunkAddr(addr, level)) return;
 ...
 *  
 *******************************************************************/
static int omCheckPrintBinError(const char* msg, 
                                void* addr, size_t size,omBin bin)
{
  fprintf(stderr, 
...
{
  omBin bin = NULL;
  const char* msg = omTestBinAddrSize(addr, size, level, &bin);
  if (msg != NULL)
    return omPrintBinError(msg, addr, size, bin);
  else
    return 1;
}
...
static const char* 
omCheckTestBinAddrSize(void* addr, size_t size, int level, omBin* r_bin)
{
  omBin bin;
  omBinPage page;
  omBin h_bin;
  ...
  if (! omIsKnownTopBin(bin))
    return omError("Addr not from Bin (Bin unknown)");
  *r_bin = bin;
  
  if ((msg = omTestBin(bin, level - 1)) != NULL) return msg;
  ...
  if ((msg = omTestBinPage(page, level - 1)) != NULL) return msg;
...
static const char* omCheckTestBin(omBin bin, int level)
{
  ...
  if (! omIsKnownTopBin(bin)) return omError("TopBin unknown");
  ...
  do
  {
    ...
    if ((msg = omTestBinPage(bin->current_page, level-1)) != NULL)
      return msg;

    if ((msg = omTestBinPage(bin->last_page, level- 1)) != NULL)
      return msg;
    ...
    while (page != NULL)
    {
      if (page != bin->last_page && page != bin->current_page &&
          ((msg = omTestBinPage(page, level - 1)) != NULL))
        return msg;
