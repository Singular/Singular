...
****************************************/
/* $Id: misc.cc,v 1.1 1998-11-19 15:16:41 krueger Exp $ */
/*
...
/*========================================================================*/
void generate_mod(procdefv pi, moddefv module, cfilesv c_filelist)
{
  procdefv v = pi;
  FILE *fp_c, *fp_h;
  char *filename;
...
void generate_function(procdefv pi, FILE *fp)
{
  ...
  if(pp!= NULL && pp->typ!=SELF_CMD) {
    ...
    fprintf(fp, "  int tok = NONE;\n");

    while(pp!=NULL) {
      fprintf(fp, "  if(v==NULL) goto mod_%s_error;\n", pi->funcname);
      fprintf(fp, "  tok = v->Typ();\n");
      fprintf(fp, "  if(tok!=%s) goto mod_%s_error;\n", 
              pp->typname, pi->funcname);
      fprintf(fp, "  v = v->next;\n");
      pp = pp->next;
    }
...
void mod_create_makefile(moddefv module, cfilesv c_filelist)
{
  ...
  fprintf(fp, "clean:\n");
  fprintf(fp, "\trm -f *.o *.lo *.so* *.la *~ core\n\n");
