...
#ifndef NDEBUG
  #if SIZEOF_LONG==8
	if(this->gcBasis!=(ideal)(0xfbfbfbfbfbfbfbfb))
...
// 		temp3=pSub(temp1, temp2);
		temp3=pSub(pCopy(H->m[ii]),pCopy(kNF(G,NULL,H->m[ii],0,0)));
 ...
 */
inline bool gcone::ivAreEqual(intvec &a, intvec &b)
{
	bool res=TRUE;
	for(int ii=0;ii<this->numVars;ii++)
	{
...
#ifdef gfan_DEBUG
				cout << "Checking facet (";fNormal->show(1,1);cout << ") against (";slNormal->show(1,1);cout << ")" << endl;
#endif				
...
#ifdef gfan_DEBUG
					cout << "Removing (";fNormal->show(1,1);cout << ") from list" << endl;
#endif
					delete slNormal;
	...
	while(fAct!=NULL)
	{
		if(fAct->isFlippable)
		{
			...
			while(slAct!=NULL)
			{
				removalOccured=FALSE;
#ifdef gfan_DEBUG
cout << "Checking facet (";fAct->fNormal->show(1,1);cout << ") against (";slAct->fNormal->show(1,1);cout << ")" << endl;
#endif	
				if(areEqual2(fAct,slAct))
				{
					facet *marker=slAct;
