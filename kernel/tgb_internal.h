...
int temp_size,SparseRow<number_type>* row, number coef)
{
  ...
  for(j=0;j<len;j=j+256)
  {
    ...
    for(i=j;i<bound;i++)
    {
      int idx=idx_array[i];
      assume(bpos<256);
      assume(!(n_IsZero((number) buffer[bpos],currRing->cf)));
      temp_array[idx]=F4mat_to_number_type(n_Add((number) temp_array[idx], (number) buffer[bpos++],currRing->cf));
...
int temp_size,const number_type* row, int len,number coef)
{
  ...
  for(j=0;j<len;j=j+256)
  {
    ...
    for(i=j;i<bound;i++)
    {
      ...
      temp_array[i]=F4mat_to_number_type(n_Add((number) temp_array[i], (number) buffer[bpos++],currRing->cf));
      assume(i<temp_size);
    }
...
int temp_size,const number_type* row, int len)
{
  ...
  for(i=0;i<len;i++)
  {
      ...
      temp_array[i]=F4mat_to_number_type(n_Sub((number) temp_array[i], (number) row[i],currRing->cf));
      assume(i<temp_size);
  }
...
template <class number_type> void add_sparse(number_type* const temp_array,int temp_size,SparseRow<number_type>* row)
{
  int j;
        ...
        {
          int idx=idx_array[j];
          temp_array[idx]=F4mat_to_number_type(   (number_type)(long)n_Add((number) temp_array[idx],(number) coef_array[j],currRing->cf));
          assume(idx<temp_size);
        }
}
template <class number_type> void sub_sparse(number_type* const temp_array,int temp_size,SparseRow<number_type>* row)
{
  int j;
        ...
        {
          int idx=idx_array[j];
          temp_array[idx]=F4mat_to_number_type(  (number_type)(long) n_Sub((number) temp_array[idx],(number) coef_array[j],currRing->cf));
          assume(idx<temp_size);
        }
}
   ...
   for(i=0;i<len;i++)
   {
     ...
     if ((red.ref))
     {
       ...
       else
       {
         ...
         {
           temp_array[red.ref->term_index]=F4mat_to_number_type( n_Add((number) temp_array[red.ref->term_index],red.coef,currRing->cf));
         }
         else
         {
           //PrintS("third case\n");
...
template<class number_type> void write_coef_times_xx_idx_to_buffer(CoefIdx<number_type>* const pairs,int& pos,int* const idx_array, number_type* const coef_array,const int rlen, const number coef)
{
  ...
  for(j=0;j<rlen;j++)
  {
    ...
    ci.coef=F4mat_to_number_type(n_Mult((number) coef,(number) coef_array[j],currRing->cf));
    ci.idx=idx_array[j];
    pairs[pos++]=ci;
  }
}
...
template <class number_type> SparseRow<number_type>* noro_red_to_non_poly_sparse(MonRedResNP<number_type>* mon, int len,NoroCache<number_type>* cache)
{
  ...
  for(i=1;i<together;i++)
  {
    ...
    {
      pairs[act].coef=F4mat_to_number_type(n_Add((number)pairs[act].coef,(number)pairs[i].coef,currRing->cf));
    }
  }
...
public:
  ...
  void multiplyRow(int row, number_type coef)
  {
    ...
    {
      row_array[i]=F4mat_to_number_type(n_Mult((number) row_array[i],(number) coef,currRing->cf));
    }
  }
  void reduceOtherRowsForward(int r)
  {
    ...
    assume(start<ncols);
    int other_row;
    assume(!(n_IsZero((number) row_array[start],currRing->cf)));
    if (!(n_IsOne((number) coef,currRing->cf)))
      multiplyRow(r,F4mat_to_number_type(n_Invers((number) coef,currRing->cf)));
    assume(n_IsOne((number) row_array[start],currRing->cf));
    ...
    for (other_row=r+1;other_row<nrows;other_row++)
    {
      ...
      if (startIndices[other_row]==start)
      {
        ...
        number coef2=n_Neg((number) other_row_array[start],currRing->cf);
        if (coef2==minus_one)
        {
          for(i=start;i<=lastIndex;i++)
          {
            if (row_array[i]!=zero)
              other_row_array[i]=F4mat_to_number_type(n_Sub((number) other_row_array[i], (number) row_array[i],currRing->cf));
          }
      }
      else
      {
          ...
          {
            if (row_array[i]!=zero)
            other_row_array[i]=F4mat_to_number_type(n_Add(n_Mult(coef2,(number) row_array[i],currRing->cf),(number) other_row_array[i],currRing->cf));
          }
        }
        updateStartIndex(other_row,start);
        assume(n_IsZero((number) other_row_array[start],currRing->cf));
      }
    }
  }
  ...
  {
    number_type* row_array=rows[row];
    assume((lower_bound<0)||(n_IsZero((number) row_array[lower_bound],currRing->cf)));
    int i;
...
//  (number_type* array, int nrows, int ncols, int* startIndices, number_type** rows){
    ...
    for(i=0;i<=nonZeroUntil;i++)
    {
      ...
      assume(!(n_IsZero((number) rows[i][startIndices[i]],currRing->cf)));
  ...
  void backwardSubstitute(int r)
  {
    ...
    assume(start<ncols);
    int other_row;
    if (!(n_IsOne((number) row_array[r],currRing->cf)))
    {
      //it should be one, but this safety is not expensive
      multiplyRow(r, F4mat_to_number_type(n_Invers((number) row_array[start],currRing->cf)));
    }
    int lastIndex=modP_lastIndexRow(row_array, ncols);
    ...
    for(other_row=r-1;other_row>=0;other_row--)
    {
      ...
      {
        number_type* other_row_array=rows[other_row];
        number coef=n_Neg((number) other_row_array[start],currRing->cf);
        assume(!(n_IsZero(coef,currRing->cf)));
        int i;
        ...
        {
          if (row_array[i]!=zero)
            other_row_array[i]=F4mat_to_number_type(n_Add(n_Mult(coef,(number)row_array[i],currRing->cf),(number)other_row_array[i],currRing->cf));
        }
