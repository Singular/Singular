...
int replaceOneSPoly (poly psi,poly p,kStrategy strat, int posInL)
{
  assume(p!=NULL);
  int      j,compare,compareCoeff;
  LObject  h;
  ...
  h.lcm = p_Lcm(p, psi, currRing);
  pSetCoeff0(h.lcm, n_Lcm(pGetCoeff(p), pGetCoeff(psi), currRing->cf));
  if (nIsZero(pGetCoeff(h.lcm)))
  {
      strat->cp++;
      pLmDelete(h.lcm);
  ...
  s = pGetCoeff(psi);
  t = pGetCoeff(p);
  k_GetLeadTerms(p, psi, currRing, m1, m2, currRing);
  ksCheckCoeff(&s, &t, currRing->cf);
  pSetCoeff0(m1, s);
  pSetCoeff0(m2, t);
  m2 = pNeg(m2);
  ...
  {
    if((pGetComp(psi) == 0) && (0 != pGetComp(p)))
    {
      p_SetCompP(sim2, pGetComp(p), strat->tailRing);
      pSetmComp(sim2);
    }
  ...
  h.p2 = p;
  #endif
  h.i_r1 = strat->L[posInL].i_r1;
  h.i_r2 = strat->L[posInL].i_r2;
  if (currRing!=strat->tailRing)
    h.t_p = k_LmInit_currRing_2_tailRing(h.p, strat->tailRing);
  /* delete old and add new pair */
  if (strat->L[posInL].lcm != NULL)
  {
      kDeleteLcm(&(strat->L[posInL]));
  }
  if (strat->L[posInL].p!=NULL)
  {
    if (pNext(strat->L[posInL].p) == strat->tail)
    {
#ifdef HAVE_RINGS
      if (pGetCoeff(strat->L[posInL].p) != NULL)
        pLmDelete(strat->L[posInL].p);
      else
#endif
        pLmFree(strat->L[posInL].p);
      /*- tail belongs to several int spolys -*/
    }
    else
    {
      // search p in T, if it is there, do not delete it
      if (rHasGlobalOrdering(currRing) || (kFindInT(strat->L[posInL].p, strat) < 0))
      {
        // assure that for global orderings kFindInT fails
        //assume((rHasLocalOrMixedOrdering(currRing)) && (kFindInT(strat->L[posInL].p, strat) >= 0));
        strat->L[posInL].Delete();
      }
    }
  }
  strat->L[posInL]  = h;
...
int replaceOneStrongPoly (poly si,poly p,kStrategy strat, int posInL)
{
  ...
  if (nIsZero(s) || nIsZero(t))  // evtl. durch divBy tests ersetzen
  {
    ...
    nDelete(&s);
    nDelete(&t);
    /* we can delete the strong spoly, it is no needed due
     * to the smaller lead coefficient in p.p */
    deleteInL(strat->L, &strat->Ll, posInL, strat);
    return 0;
  }
